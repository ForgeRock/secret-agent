...
[![PkgGoDev](https://pkg.go.dev/badge/github.com/ForgeRock/secret-agent)](https://pkg.go.dev/github.com/ForgeRock/secret-agent) [![Go Report Card](https://goreportcard.com/badge/github.com/ForgeRock/secret-agent)](https://goreportcard.com/report/github.com/ForgeRock/secret-agent)

![Secret agent logo a go gopher with sunglasses and hawaiian style shirt](/assets/secretagent.png)

The `secret-agent` is a Kubernetes operator that generates the secrets required by the ForgeRock Platform. These secrets are stored in-cluster as Kubernetes secrets and can also be stored in a Cloud Secret Manager.

## Installation

To install the latest release `secret-agent` in a kubernetes environment, run:
...
It is important to note that the kubernetes secrets produced by the `secret-agent` will be placed in the same namespace as the SAC. If the similar secrets are desired in multiple namespaces, one SAC would be required per namespace.
...
If a cloud provider has been configured, the operator will attempt to load the secret data from that cloud provider. If the secret is found in the cloud provider's secret manager, the operator will use the found data as the k8s secret data. The operator will only generate new secrets if no secret data is found in the cloud provider.

The `secret-agent` supports the following cloud providers:

* Google Secret Manager
* AWS Secrets Manager
* ~~Azure Key Vault~~ (In progress)

It is possible to run the `secret-agent` without setting up a cloud provider. This is especially useful when debugging or testing applications. To disable cloud provider support set `spec.appConfig.secretsManager` to “none”.
...
* EC2 Instance Metadata (preferred): _Obtains credentials form 169.254.169.254_

Please refer to [AWS documentation](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access_overview.html) for instructions on how to obtain credentials and grant necessary permissions to access the AWS Secrets Manager. The `secret-agent` needs access read/write secrets. This can be achieved by allowing access to the `arn:aws:iam::aws:policy/SecretsManagerReadWrite` AWS managed policy.
...
All that is required is to provide a Kubernetes secret with the same name and same key names as described in the SAC. It is important to note that if the [cloud backup](#enabling-cloud-backup) feature is enabled, the secret to be imported must be provided via the cloud manager's secret manager. The `secret-agent` will ignore local secrets if [cloud backup](#enabling-cloud-backup) is enabled.

### Naming Convention For Cloud Backups

There is a naming convention used by `secret-agent` to store and read secrets from the cloud secret managers. In general, the names used follow the following format:
...
Due to cloud providers limitation, all `/`, `.` and `_` in the secret names are replaced by `-` when accessing the cloud secret managers.

For example, see the following Secret Agent Configuration:
...
Where `$NAME` follows the convention at the top of this section.
...
--- | --- | ---
`spec.appConfig.createKubernetesObjects` | Creates k8s secrets for each generated secret  | true
...
`spec.appConfig.credentialsSecretName` | Name of the k8s secret containing the credentials to access the cloud provider | ""
...
`spec.secrets` | List of k8s secrets to create. See [Secret Config](#secret-config) | []

### Secret Config

Parameter | Description | Default
--- | --- | ---
`name` | Name of the k8s secret to generate | ""
`keys` | list of the specs of each key in the k8s secret. See [Key Config](#key-config) | []

### Key Config

Parameter | Description | Default
--- | --- | ---
`name` | Name of the key in the k8s secret | ""
...
`spec.algorithm` | Used when key type is `keyPair`. Specify the algo used to generate the keyPair | ""
